...
extension Character {
    /*  В Swift нет способа узнать, является ли Character Emoji, но он позволяет нам проверить, являются ли скаляры наших компонентов isEmoji. К сожалению, Unicode позволяет некоторым скалярам (например, 1) изменяться другим скаляром, чтобы они стали эмодзи (например, 1️⃣), поэтому скаляр «1» сообщит isEmoji = true. Поэтому мы не можем просто проверить, является ли первый скаляр isEmoji. Самый простой способ — проверить, является ли скаляр хотя бы первым известным нам истинным эмодзи (начало раздела «Разное»), или проверить, является ли это множественной скалярной последовательностью Unicode (например, 1 с модификатором Unicode, чтобы принудительно представить его как эмодзи 1️⃣)*/
    var isEmoji: Bool {
